...
│  │  └─ customLabelsWorkflow.js # Custom Labels 학습/추론 파이프라인
│  ├─ lambda/
│  │  ├─ uploadFacesHandler.js           # Lambda: face1~6 S3 업로드
│  │  ├─ compareFacesHandler.js          # Lambda: face1~6 유사도 비교
│  │  ├─ trainCustomModelHandler.js      # Lambda: Custom Labels 프로젝트 생성 + 학습 시작
│  │  └─ detectCustomLabelsHandler.js    # Lambda: Custom Labels 추론
│  ├─ training/
│  │  └─ custom-labels-manifest.jsonl   # 예시 학습 매니페스트
...
- 최소 권한 권장 정책
  ...
  - Custom Labels: `CreateProject`, `CreateDataset`, `CreateProjectVersion`, `StartProjectVersion`, `DetectCustomLabels`, `StopProjectVersion`
...
- `LAMBDA_CUSTOM_TRAIN_FUNCTION`: Custom Labels 학습 Lambda 함수명 기본값
- `LAMBDA_CUSTOM_PREDICT_FUNCTION`: Custom Labels 추론 Lambda 함수명 기본값
- `CUSTOM_LABELS_PROJECT`: Custom Labels 프로젝트 이름
- `CUSTOM_LABELS_MANIFEST_KEY`: 매니페스트 S3 키 (기본 `custom-labels/manifest.jsonl`)
- `CUSTOM_LABELS_OUTPUT_PREFIX`: 학습 결과 S3 프리픽스
- `CUSTOM_LABELS_VERSION_ARN`: 추론 대상 모델 ARN (`custom-labels-predict` 필수)
- `CUSTOM_LABELS_IMAGE_KEY`: 추론할 이미지 S3 키
...
## 8-1) 도메인 Fine-tuning — Custom Labels 실습

> 상세 가이드: `DOC/Chapter11/Chapter11.md`

Amazon Rekognition Custom Labels를 사용하면 **도메인 특화 이미지**로 Fine-tuning된 모델을 학습하고, 해당 모델로 커스텀 라벨을 추론할 수 있습니다.
...
# 2) 매니페스트 편집 후 학습 시작 (30분~수 시간 소요)
./scripts/aws_batch_ops.sh custom-labels-train

# 3) 학습 상태 확인 (StatusMessage가 TRAINING_COMPLETED 될 때까지)
PROJECT_ARN=$(cat batch-work/custom-train-result.json | python3 -c "import sys,json; print(json.load(sys.stdin)['projectArn'])")
VERSION_NAME=$(cat batch-work/custom-train-result.json | python3 -c "import sys,json; print(json.load(sys.stdin)['versionName'])")
aws rekognition describe-project-versions \
  --project-arn "${PROJECT_ARN}" \
  --version-names "${VERSION_NAME}" \
  --region "${AWS_REGION}" \
  --query "ProjectVersionDescriptions[0].{Status:Status,F1:EvaluationResult.F1Score}" \
  --output table

# 4) 학습 완료 후 추론 실행
export CUSTOM_LABELS_VERSION_ARN=$(cat batch-work/custom-train-result.json | python3 -c "import sys,json; print(json.load(sys.stdin)['projectVersionArn'])")
...
```bash
cd server
npm run custom-labels:train    # 학습 시작
npm run custom-labels:predict  # 추론 (CUSTOM_LABELS_VERSION_ARN 필요)
```
...
|---|---|
| 학습 | 시간당 약 $1.00 |
| 모델 호스팅 | 시간당 약 $4.00/추론 유닛 |

추론 후에는 반드시 모델을 중지하세요. `detectCustomLabelsHandler.js`가 추론 후 자동으로 `StopProjectVersion`을 호출합니다.
